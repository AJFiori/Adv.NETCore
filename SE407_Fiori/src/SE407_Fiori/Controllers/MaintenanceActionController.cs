...
namespace SE407_Fiori.Controllers
{
    public class MaintenanceActionController : Controller
    {
        ...
        {
            MaintenanceActionViewModel maDeleteVM = new MaintenanceActionViewModel();
            using (MaintenanceActionDBContext db = new MaintenanceActionDBContext())
            {
                using (var dbma = new MaintenanceRecordDBContext())
                {
                    MaintenanceRecordViewModel maVM = new MaintenanceRecordViewModel();
                    maVM.MaintenanceRecordList = dbma.MaintenanceRecords.ToList();
                    maVM.NewMaintenanceRecord = dbma.MaintenanceRecords.Where(
                    b => b.MaintenanceActionId == id).FirstOrDefault();
                    //instantiate object from view model
                    if (maVM.NewMaintenanceRecord == null)
                    {
                        maDeleteVM.NewMaintenanceAction = new MaintenanceAction();
                        //retrieve primary key/id from route data
                        maDeleteVM.NewMaintenanceAction.MaintenanceActionId =
                            Guid.Parse(RouteData.Values["id"].ToString());
                        //mark the record as modified
                        db.Entry(maDeleteVM.NewMaintenanceAction).State =
                            EntityState.Deleted;
                        //persist changes
                        db.SaveChanges();
